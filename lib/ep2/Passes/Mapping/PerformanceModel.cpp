...
NetronomeSpec NetronomeSpec::load(llvm::StringRef path) {
  ...
  //                  "latency_cycles": 4}, ...]
  if (const llvm::json::Array *mls = root->getArray("memory_layers")) {
    for (const auto &elem : *mls) {
      const llvm::json::Object *ml = elem.getAsObject();
      if (!ml) continue;
      MemLayerSpec mls;
      if (auto v = ml->getString("id"))     mls.id    = v->str();
      if (auto v = ml->getString("scope"))  mls.scope = v->str();
      if (auto v = ml->getInteger("size_bytes"))     mls.sizeBytes    = *v;
      if (auto v = ml->getInteger("latency_cycles")) mls.latencyCycles = (int)*v;
      s.memoryLayers.push_back(std::move(mls));
    }
  }
...
PerformanceModel::getMapping(llvm::SmallVector<ep2::FuncOp> &ops) {
  ...
  for (auto unit : units) {
    ...
    for (int idx = 0; idx < ops.size(); idx++) {
      ...
      double perReplica = static_cast<double>(totalLatency) / replicas;
      double deficit = perReplica - latencyTarget;

      // only consider stages that still exceed the target
