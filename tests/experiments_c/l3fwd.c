...
// Routing tables: value = (egress_port, valid-bit)
struct rt_val_t {
...
#define L3SW_FLAG_MAC_REWRITTEN 2  // bit1
#define L3SW_FLAG_LPM_MODE      4  // bit2

// Dispatcher metadata
// flags: bit0=drop, bit1=mac_rewritten, bit2=lpm_mode
struct l3sw_meta_t {
  BITS_FIELD(32, egress_port);
  BITS_FIELD(32, flags);
  BITS_FIELD(32, ingress_port);
...
void NET_RECV__process_packet(buf_t packet) {
  ...
  outm.flags        = cfg.mac_rewrite * L3SW_FLAG_MAC_REWRITTEN
                    + use_lpm         * L3SW_FLAG_LPM_MODE;
